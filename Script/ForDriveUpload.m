...
omega_1 = diff(theta,t)*cos(phi) + diff(psi,t)*(sin(theta)*sin(phi))
omega_2 = diff(psi,t)*(sin(theta)*cos(phi)) - diff(theta,t)*sin(phi)
...
ShallBeZero = simplify(T_exp_Holder - 0.5*dq_Holder'*M*dq_Holder) %!!!
...
ct_1 = (dX == R*diff(phi,t)*cos(psi))
ct_2 = (dY == R*diff(phi,t)*sin(psi))
