...
momentInertiaB = [9.2 2.8 11];                  % Rear Frame Moment of inertia (kg-m^2)
ProductsofInertiaB = [0 0 2.4];                 % Rear Frame products of inertia (kg-m^2)

momentInertiaH = [0.05892 0.00708 0.06];        % Front frame Moment of inertia (kg-m^2)
ProductsofInertiaH = [0 0 -0.00756];            % Front frame products of inertia (kg-m^2)

CenterOfMassB_in_I = [0.4;0;0.6];               % Input Center of Mass here in global reference frame(Rear Frame)
% (Not to be modified!!)
...
CenterOfMassH_in_I = [1.0;0;0.7];               % Input Center of Mass here in global reference frame(Front Frame)
% (Not to be modified!!)
...
% 1. Rear Wheel
k_contact_RW = 1e7;         % Vertical stiffness (N/m)
b_contact_RW = 2e3;         % Damping (N-s/m)

% 2. Front wheel
k_contact_FW = 1e7;        % Vertical stiffness (N/m)
b_contact_FW = 2e3;        % Damping (N-s/m)

% 2. Stiffness properties

C_F_alpha = 2e3;          % Longitudinal stiffness
C_F_kappa = 2e3;          % Lateral stiffness
